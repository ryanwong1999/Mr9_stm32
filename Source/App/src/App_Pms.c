...
void Read_PMUData(void)	
{
  ...
  if(abs(ADC_Filter_Value[3] - 2000) < 5) Pms.Bat_Current = 0;
	else Pms.Bat_Current = (ADC_Filter_Value[3] - 2000) * 18.75;		//电池电流
	//计算电池容量
...
uint8_t Get_Batt_Capacity(int16_t _bat_cur,uint16_t _bat_vol)
{
	...
	if(voltage_cnt >= 60)
	{
	  ...
	  cur_tmp = current_sum/60;		//1分钟的平均值
		...
		if(cur_tmp < 0 && Pms.Capacity_mah < abs(cur_tmp/60)) Pms.Capacity_mah = 0;
		else Pms.Capacity_mah += cur_tmp/60;
		//判断开始充电的时候电压低于20%，直到充满
		if(vol_tmp <= 25040 && (Pms.Bat_Sta & 0x11) != 0)
		{
			//自动校准容量标志位
			Pms.Reset_Flag = 1;
		}
		//非充电中没电了
		if(vol_tmp < EMPTY_VOL && (Pms.Bat_Sta & 0x01) == 0)
		{
		...
		//没电了
		else if(Pms.Capacity_mah <= EMPTY_CAP)
		{
			Pms.Capacity_mah = EMPTY_CAP;
		}
		else 
